...
# Narrow workspace to web only — the lockfile references backend/extension
# but their package.jsons aren't in the Docker context (.dockerignore).
RUN printf 'packages:\n  - "web"\n' > pnpm-workspace.yaml
...
COPY . .
# Narrow workspace to web only (same as deps stage)
RUN printf 'packages:\n  - "web"\n' > pnpm-workspace.yaml
