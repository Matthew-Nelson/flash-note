...
import { tryMarkWebhookProcessed, cleanupOldWebhookEvents, deleteProcessedWebhookEvent } from './webhooks';
...
describe('Webhook Queries', () => {
  ...
  describe('cleanupOldWebhookEvents', () => {
    ...
    it('should reject daysToKeep less than 1', async () => {
      await expect(cleanupOldWebhookEvents(0)).rejects.toThrow(
        'cleanupOldWebhookEvents: daysToKeep must be >= 1'
      );
      await expect(cleanupOldWebhookEvents(-5)).rejects.toThrow(
        'cleanupOldWebhookEvents: daysToKeep must be >= 1'
      );
    });
