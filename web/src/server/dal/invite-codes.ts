...
import type { InviteCodeRow } from '@/lib/types/database';
import { generateCodeString } from '@/server/lib/invite-code-format';

// Re-export for consumers that need code generation without DB dependency
export { generateCodeString } from '@/server/lib/invite-code-format';
...
export async function createInviteCode(
  ...
  if (result.rows.length === 0) {
    throw new Error('createInviteCode: INSERT RETURNING returned no rows');
  }
