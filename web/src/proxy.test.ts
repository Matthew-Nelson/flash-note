...
describe('Proxy', () => {
  ...
  describe('CSP headers', () => {
    it('should set CSP header on response', () => {
      ...
      const response = proxy(createRequest('https://flashnote.co/'));
    ...
    it('should generate unique nonces per request', () => {
      vi.stubEnv('NODE_ENV', 'production');
      const response1 = proxy(createRequest('https://flashnote.co/'));
      const response2 = proxy(createRequest('https://flashnote.co/'));
  ...
  describe('auth redirect', () => {
    it('redirects /dashboard to /login when no session_id cookie', () => {
      vi.stubEnv('NODE_ENV', 'production');
      const response = proxy(createRequest('https://flashnote.co/dashboard'));
    ...
    it('redirects /dashboard/settings to /login when no cookie', () => {
      vi.stubEnv('NODE_ENV', 'production');
      const response = proxy(createRequest('https://flashnote.co/dashboard/settings'));
      expect(response.status).toBe(307);
      expect(response.headers.get('Location')).toBe('https://flashnote.co/login');
    });
    ...
    it('does NOT redirect public routes without cookie', () => {
      ...
      for (const route of publicRoutes) {
        const response = proxy(createRequest(`https://flashnote.co${route}`));
        expect(response.status).not.toBe(307);
      }
    });
    ...
    it('does NOT redirect /login when no session_id cookie', () => {
      vi.stubEnv('NODE_ENV', 'production');
      const response = proxy(createRequest('https://flashnote.co/login'));
      expect(response.status).not.toBe(307);
    });

    it('does NOT redirect /signup when no session_id cookie', () => {
      vi.stubEnv('NODE_ENV', 'production');
      const response = proxy(createRequest('https://flashnote.co/signup'));
      expect(response.status).not.toBe(307);
    });
    ...
    it.each([
      ...
      'session_limit',
      'session_revoked',
    ] as const)('clears stale cookie and allows /login when ?reason=%s with cookie', (reason) => {
      vi.stubEnv('NODE_ENV', 'production');
      const response = proxy(
        createRequest(`https://flashnote.co/login?reason=${reason}`, { cookies: { session_id: 'stale-token' } })
      );
